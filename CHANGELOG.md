...
- WEF/WEC central collection: `New-WefSubscription.ps1` generates a
  ...
  channel), with collector (`wecutil`) and source (GPO SubscriptionManager)
  setup guidance printed. The kit's pipeline boundary is documented:
  generate (kit) -> transport (WEF/WEC) -> ingest (SIEM, out of scope).
- Reference baseline presets in `presets/`: ASD, Microsoft_Client and
  ...
  parameter, faithful to the scripts in Yamato's EventLog-Baseline-Guide,
  with documented faithfulness limits. Regenerated by
  `tools/New-PresetBaselines.ps1` and drift-checked in CI.
