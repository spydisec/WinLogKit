...
- 🔒 **Test says when the AppLocker logs can't record anything.** Those logs pass as enabled, but only record while an AppLocker policy is in effect. When they're selected, Test now notes if no AppLocker policy is in effect, or if AppLocker isn't available on the Windows edition, so empty logs aren't mistaken for coverage. Read-only: the kit never creates an AppLocker policy. [#74](https://github.com/spydisec/WinLogKit/issues/74)
