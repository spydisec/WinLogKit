...
remediations** (`New-IntuneRemediationPack.ps1`) and central collection via
**WEF/WEC subscriptions** (`New-WefSubscription.ps1`) are built in; GPO-based
audit policy delivery is on the [roadmap](ROADMAP.md). Versioned releases with
...
| `New-WefSubscription.ps1` | Generates a source-initiated WEF subscription XML from the settings table or a baseline CSV, so the channels the kit enables can be collected centrally on a Windows Event Collector. Prints the collector (`wecutil`) and source (GPO SubscriptionManager) setup steps. |
| `presets/` | Reference baselines expressed as selection CSVs - `ASD.csv`, `Microsoft_Client.csv`, `Microsoft_Server.csv` - usable anywhere a `-BaselineFile` is accepted. Regenerated by `tools/New-PresetBaselines.ps1`; CI fails if they drift from the settings table. |
