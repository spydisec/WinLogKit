...
| `Export-AttackCoverage.ps1` | Reports which MITRE ATT&CK techniques a selection makes observable and why the rest are not, using the kit's native mapping: current ATT&CK v19.2 analytics data joined through a kit-curated event map (`data/attack/`). MITRE's analytics put the native-logging ceiling at 284 of 472 Windows techniques; +HighVolume reaches 279 of it. `-UseOssem` runs the [OSSEM-DM](https://github.com/OTRF/OSSEM-DM) snapshot as a cross-check (approach credit in `data/attack/README.md`). |
| `presets/` | Selection CSVs usable anywhere a `-BaselineFile` is accepted: reference baselines (`ASD.csv`, `Microsoft_Client.csv`, `Microsoft_Server.csv`), per-role starting points (`role_*`), and the **spydi blended baselines** - ASD + Microsoft + Yamato on two axes, role (Server incl. DCs/WEF collectors, Workstation) x volume (Minimal, Heavy): Workstation 263/269, Server 273/**279** of 472 ATT&CK techniques (284 native ceiling; sources and events per group in the docs). Regenerated by `tools/New-PresetBaselines.ps1`; CI fails on drift. |
