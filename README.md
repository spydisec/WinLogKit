...
| `presets/` | Selection CSVs usable anywhere a `-BaselineFile` is accepted: reference baselines (`ASD.csv`, `Microsoft_Client.csv`, `Microsoft_Server.csv`) and per-role starting points (`role_Workstation.csv` 265/472 ATT&CK techniques, `role_MemberServer.csv` 263, `role_DomainController.csv` 273 - against a 284 native ceiling; rationale in the docs). Regenerated by `tools/New-PresetBaselines.ps1`; CI fails if they drift from the settings table. |
