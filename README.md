...
| `Export-AttackCoverage.ps1` | Reports which MITRE ATT&CK techniques a selection makes observable, and why the rest are not (deselected / excluded by design / Sysmon-only), by joining the vendored [OSSEM-DM](https://github.com/OTRF/OSSEM-DM) snapshot in `data/ossem/`. Core = 282 of 362 mapped Windows techniques; +HighVolume = 320. |
