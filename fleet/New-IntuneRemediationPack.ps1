...
# run as SYSTEM (no profile, no prompts; the only file written is a temporary
# audit policy export, deleted straight after reading).
...
# Audit policy is read from auditpol /backup's numeric setting values, so the
# checks work on any Windows display language.
...
# GUID -> setting value (0 none, 1 Success, 2 Failure, 3 both), read by
# column position from a temporary auditpol export that is deleted at once.
$auditMap = @{}
$auditFile = Join-Path ([IO.Path]::GetTempPath()) ('winlogkit-auditpol-{0}.csv' -f [guid]::NewGuid())
try {
    ...
    try {
        ...
        switch ($i.Type) {
            ...
            'AuditPolicy' {
                $checked++
                $cur = 0
                if ($auditMap.ContainsKey($i.Guid)) { $cur = [int]$auditMap[$i.Guid] }
                $ok = $true
                if ($i.Success -and ($cur -band 1) -eq 0) { $ok = $false }
                if ($i.Failure -and ($cur -band 2) -eq 0) { $ok = $false }
                if (-not $ok) {
                    if ($Mode -eq 'Detect') {
