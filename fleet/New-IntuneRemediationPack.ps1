...
foreach ($rs in $script:BaselineRegistrySettings) {
    ...
    # The folder item precedes its registry value, as in Enable-LoggingBaseline.
    if ($rs.ContainsKey('CreateFolder') -and $rs.CreateFolder -and -not $folderPaths.ContainsKey($rs.Value)) {
        $folderPaths[$rs.Value] = $true
        $lines.Add(('    @{{ Type=''Folder''; Path={0}; DCOnly={1} }}' -f `
            (ConvertTo-PsString $rs.Value), (ConvertTo-PsBool ($rs.Scope -eq 'DomainController'))))
    }
...
# run as SYSTEM (no profile, no prompts, no files written beyond creating the
# transcript folder when transcription is selected).
...
# Transcript folder drop-box ACL (from the kit's settings table).
$FolderOwner = '__FOLDEROWNER__'
$FolderAcl = @(
__FOLDERACL__
)
function New-FolderAce { param($e)
    New-Object System.Security.AccessControl.FileSystemAccessRule((New-Object System.Security.Principal.SecurityIdentifier $e.Sid),
        [System.Security.AccessControl.FileSystemRights]$e.Rights, [System.Security.AccessControl.InheritanceFlags]$e.Inherit,
        [System.Security.AccessControl.PropagationFlags]$e.Propagate, [System.Security.AccessControl.AccessControlType]::Allow)
}
function Test-FolderOk { param([string]$Path)
    if (-not (Test-Path -LiteralPath $Path -PathType Container)) { return $false }
    $sidType = [System.Security.Principal.SecurityIdentifier]
    $acl = Get-Acl -LiteralPath $Path
    if ($acl.GetOwner($sidType).Value -ne $FolderOwner -or -not $acl.AreAccessRulesProtected) { return $false }
    $key = { param($r) '{0}|{1}|{2}|{3}|{4}' -f $r.IdentityReference.Value, [int]$r.FileSystemRights, [int]$r.InheritanceFlags, [int]$r.PropagationFlags, $r.AccessControlType }
    $want = @($FolderAcl | ForEach-Object { & $key (New-FolderAce $_) } | Sort-Object)
    $have = @($acl.GetAccessRules($true, $false, $sidType) | ForEach-Object { & $key $_ } | Sort-Object)
    return (($want -join ';') -eq ($have -join ';'))
}
function Set-FolderAcl { param([string]$Path)
    if (-not (Test-Path -LiteralPath $Path -PathType Container)) { New-Item -ItemType Directory -Path $Path -Force | Out-Null }
    $acl = New-Object System.Security.AccessControl.DirectorySecurity
    $acl.SetOwner((New-Object System.Security.Principal.SecurityIdentifier $FolderOwner))
    $acl.SetAccessRuleProtection($true, $false)
    foreach ($e in $FolderAcl) { $acl.AddAccessRule((New-FolderAce $e)) }
...
foreach ($i in $Items) {
    try {
        ...
        switch ($i.Type) {
            ...
            'Registry' {
                $checked++
                # Never point transcription at a folder that failed hardening.
                if ($badFolders.ContainsKey("$($i.Value)")) { throw "folder $($i.Value) could not be hardened; $($i.Name) not written" }
            ...
            'Folder' {
                $checked++
                if (-not (Test-FolderOk -Path $i.Path)) {
                    if ($Mode -eq 'Detect') {
                        $failList.Add("Folder:$($i.Path)")
                    } else {
                        try { Set-FolderAcl -Path $i.Path } catch { $badFolders[$i.Path] = $true; throw }
                        $fixed++
                    }
                }
            }
...
$folderAclBlock = ($script:BaselineTranscriptFolderAcl | ForEach-Object {
    '    @{{ Sid={0}; Rights={1}; Inherit={2}; Propagate={3} }}' -f `
        (ConvertTo-PsString $_.Sid), (ConvertTo-PsString $_.Rights), (ConvertTo-PsString $_.Inherit), (ConvertTo-PsString $_.Propagate)
}) -join "`r`n"
...
foreach ($mode in @('Detect', 'Remediate')) {
    ...
    $content = $template.
        ...
        Replace('__FOLDEROWNER__', $script:BaselineTranscriptFolderOwner).
        Replace('__FOLDERACL__', $folderAclBlock).
