...
- **The catalog only covers part of the baseline.** Audit policy,
  ...
  `Test-LoggingBaseline.ps1` reports those rows as FAIL on a device managed
  only through the catalog; that's expected, and running
  `Enable-LoggingBaseline.ps1` once on the device covers them.
- **CSPs aren't role-aware.** Rows marked **(DC)** only matter on domain
  controllers: a Settings catalog profile applies to every device it's
  assigned to, so leave those rows out of profiles for other devices.
...
domain-joined hosts, see [Group Policy paths](gpo-paths.md)).
